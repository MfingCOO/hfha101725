  ...
  (
    ...
    return (
      <aside
        ref={ref}
        className={cn(
          "peer hidden md:block text-sidebar-foreground transition-[width] duration-300",
          "w-[--sidebar-width]",
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="footer"
      className={cn("flex flex-col gap-2 mt-auto border-t", className)}
      {...props}
    />
  )
})
      ...
      <>
        {props.children && React.Children.map(props.children, (child) => {
            ...
            if (React.isValidElement(child) && child.type === 'span') {
                return React.cloneElement(child as React.ReactElement, {
                     className: cn("truncate transition-opacity", state === "collapsed" && !isMobile && "opacity-0 w-0")
                });
            }
            return child;
        })}
    ...
    const button = (
      <Comp
        ...
        className={cn(
            ...
            state === "collapsed" && !isMobile && "w-10 justify-center",
            className
        )}
        {...props}
      >
