...
import { formatInUserTimezone, convertToUTC } from '@/services/time';
...
export function BookingDialog({ isOpen, onClose }: BookingDialogProps) {
  ...
  useEffect(() => {
    async function fetchCoaches() {
      ...
      if (result.success && result.data) {
        ...
        if (filteredCoaches.length > 0) setSelectedCoachId(filteredCoaches[0].uid);
      }
    }
    if (isOpen) {
      fetchCoaches();
  ...
  const availableSlots = useMemo(() => {
    ...
    for (const slot of todaySettings.slots) {
      ...
      let currentSlotTime = convertToUTC(startString, availability.timezone);
      const endTime = convertToUTC(endString, availability.timezone);

      while (currentSlotTime < endTime) {
        const slotEnd = addMinutes(currentSlotTime, 15);
        const isOverlapping = existingEvents.some(event =>
          areIntervalsOverlapping({ start: currentSlotTime, end: slotEnd }, { start: new Date(event.start), end: new Date(event.end) })
        );
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const eventData = {
        ...
        // These fields were missing - this was causing silent failure
        durationDays: 0,
        durationHours: 0,
        durationMinutes: 15,
      };

      const result = await saveCalendarEvent(eventData as any);
