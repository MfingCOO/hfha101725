...
const scheduledPillarSchema = z.object({
  ...
  notes: z.string().optional(),
    ...
    if (data.recurrenceType === 'custom' && (!data.recurrenceInterval || data.recurrenceInterval <= 0)) {
         ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Interval must be at least 1.", path: ["recurrenceInterval"] });
    }
});
...
interface CreateChallengeDialogProps {
  ...
  initialData?: ChallengeFormValues | any | null; 
}
...
export function CreateChallengeDialog({ open, onOpenChange, onChallengeUpserted, initialData }: CreateChallengeDialogProps) {
  ...
  const { fields: scheduledPillarFields, append: appendScheduledPillar, remove: removeScheduledPillar } = useFieldArray({ control: form.control, name: 'scheduledPillars' });
  const { fields: customTaskFields, append: appendCustomTask, remove: removeCustomTask } = useFieldArray({ control: form.control, name: 'customTasks' });
  const { fields: scheduledHabitFields, append: appendScheduledHabit, remove: removeScheduledHabit } = useFieldArray({ control: form.control, name: 'scheduledHabits' });
                    ...
                    <div className="space-y-6 py-4">
                        <FormField control={form.control} name="name" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Input placeholder="e.g., Hydration Harmony Quest" {...field} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                        <FormField control={form.control} name="description" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Textarea placeholder="e.g., Track hydration to reduce cravings and boost energy!" {...field} maxLength={1000} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                        ...
                        <FormItem>
                            <div className="grid grid-cols-[1fr_auto] items-center gap-4">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => appendScheduledPillar({ pillarId: '', days: [], recurrenceType: 'weekly', notes: '' })}>
                                    <PlusCircle className="mr-2 h-4 w-4" />Add Pillar Block
                                </Button>
                            </div>
                            <div className="space-y-2 rounded-md border p-4">
                                 {scheduledPillarFields.length === 0 ? <p className="text-sm text-center text-muted-foreground py-4">No scheduled pillars added yet.</p> : scheduledPillarFields.map((field, index) => (
                                            ...
                                            <FormField control={form.control} name={`scheduledPillars.${index}.pillarId`} render={({ field }) => (
                                                <FormItem className="flex-1">
                                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                        ...
                                                        <SelectContent>{pillarsAndTools.filter(p => p.id !== 'insights').map(p => { const Icon = p.icon || Star; return (<SelectItem key={p.id} value={p.id}><span className="flex items-center gap-2"><Icon className="h-4 w-4"/> {p.label}</span></SelectItem>)})}</SelectContent>
                                                    </Select>
                                                    <FormMessage />
                                                </FormItem>
                                            )} />
                                            <Button type="button" variant="ghost" size="icon" className="text-destructive h-8 w-8" onClick={() => removeScheduledPillar(index)}><Trash2 className="h-4 w-4" /></Button>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <FormField control={form.control} name={`scheduledPillars.${index}.recurrenceType`} render={({ field }) => (
                                                <FormItem className="space-y-2">
                                                    ...
                                                    <Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select recurrence..."/></SelectTrigger></FormControl><SelectContent><SelectItem value="weekly">Weekly</SelectItem><SelectItem value="custom">Custom Interval</SelectItem></SelectContent></Select>
                                                </FormItem>
                                            )} />
                                            <FormField control={form.control} name={`scheduledPillars.${index}.recurrenceInterval`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Repeat every (days)</FormLabel><FormControl><AppNumberInput placeholder="e.g., 14" disabled={watchPillars?.[index]?.recurrenceType !== 'custom'} {...field} /></FormControl><FormMessage /></FormItem>)}/>
                                        </div>
                                         <FormField control={form.control} name={`scheduledPillars.${index}.notes`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Notes/Instructions</FormLabel><FormControl><Textarea placeholder="e.g., Focus on hitting your protein goal today." {...field} /></FormControl><FormMessage /></FormItem>)}/>
                                    </div>
                                 ))}
                            </div>
                        </FormItem>
                        ...
                        <FormItem>
                            <div className="grid grid-cols-[1fr_auto] items-center gap-4">
                                ...
                                <Button type="button" variant="outline" size="sm" onClick={() => appendScheduledHabit({ habitId: '', days: [], recurrenceType: 'weekly', notes: '' })}><Sparkles className="mr-2 h-4 w-4" />Add Habit Block</Button>
                            </div>
                            <div className="space-y-2 rounded-md border p-4">
                                 {scheduledHabitFields.length === 0 ? <p className="text-sm text-center text-muted-foreground py-4">No scheduled habits added yet.</p> : scheduledHabitFields.map((field, index) => (
                                    <div key={field.id} className="p-3 rounded-md bg-muted/50 border space-y-3">
                                        <div className="flex justify-between items-start gap-2">
                                            <FormField control={form.control} name={`scheduledHabits.${index}.habitId`} render={({ field }) => (
                                                <FormItem className="flex-1">
                                                    <Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select a habit..."/></SelectTrigger></FormControl><SelectContent>{customHabits.map(habit => <SelectItem key={habit.id} value={habit.id}>{habit.name}</SelectItem>)}</SelectContent></Select>
                                                    <FormMessage />
                                                </FormItem>
                                            )} />
                                            <Button type="button" variant="ghost" size="icon" className="text-destructive h-8 w-8" onClick={() => removeScheduledHabit(index)}><Trash2 className="h-4 w-4" /></Button>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <FormField control={form.control} name={`scheduledHabits.${index}.recurrenceInterval`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Repeat every (days)</FormLabel><FormControl><AppNumberInput placeholder="e.g., 14" disabled={watchHabits?.[index]?.recurrenceType !== 'custom'} {...field} /></FormControl><FormMessage /></FormItem>)}/>
                                        </div>
                                        <FormField control={form.control} name={`scheduledHabits.${index}.notes`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Notes/Instructions</FormLabel><FormControl><Textarea placeholder="e.g., Take a moment to reflect on why this habit is important to you." {...field} /></FormControl><FormMessage /></FormItem>)}/>
                                    </div>
                                 ))}
                            </div>
                        </FormItem>
                        ...
                        <FormItem>
                            ...
                            <div className="space-y-2 rounded-md border p-4">
                                {customTaskFields.length === 0 ? <p className="text-sm text-center text-muted-foreground py-4">No custom exercises added yet.</p> : customTaskFields.map((field, index) => (
                                    <div key={field.id} className="flex flex-col gap-2 p-2 rounded-md bg-muted/50 border">
                                        <div className="flex justify-end"><Button type="button" variant="ghost" size="icon" className="text-destructive h-8 w-8" onClick={() => removeCustomTask(index)}><Trash2 className="h-4 w-4" /></Button></div>
                                        <div className="space-y-3">
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                <FormField control={form.control} name={`customTasks.${index}.description`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Description</FormLabel><FormControl><Input placeholder="e.g., Push Ups" {...field} /></FormControl><FormMessage /></FormItem>)} />
                                                <FormField control={form.control} name={`customTasks.${index}.startDay`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Start Day</FormLabel><FormControl><AppNumberInput placeholder="1" {...field} /></FormControl><FormMessage /></FormItem>)} />
                                            </div>
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            ...
                                            <FormField control={form.control} name={`customTasks.${index}.notes`} render={({ field }) => (<FormItem><FormLabel className="text-xs">Modification Notes</FormLabel><FormControl><Textarea placeholder="e.g., If this is too hard, try knee push-ups." {...field} /></FormControl><FormMessage /></FormItem>)}/>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="p-4 border rounded-md space-y-4">
                                <FormField control={form.control} name="thumbnailUrl" render={({ field }) => (
                                    <FormItem>
                                        <FormControl><Input type="file" accept="image/*" onChange={handleFileSelect} className="file:text-primary file:font-semibold" /></FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )} />
                                {thumbnailPreview ? <div className="relative w-full h-40 rounded-lg overflow-hidden border"><Image src={thumbnailPreview} alt="Thumbnail preview" fill className="object-cover" unoptimized /></div> : <div className="w-full h-40 rounded-lg bg-muted flex items-center justify-center"><ImageIcon className="h-12 w-12 text-muted-foreground" /></div>}
                            </div>
                        </div>
                        ...
                        <FormField control={form.control} name="notes" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Textarea placeholder="e.g., Week 1: Perform as written.&#10;Week 2: Double all reps." {...field} maxLength={1000}/></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                    </div>
