...
export function FoodCacheModal({ isOpen, onClose, fdcId, mode, isCoach, user }: FoodCacheModalProps) {
  ...
  useEffect(() => {
    const loadFoodData = async () => {
      ...
      if (mode === 'edit' && fdcId) {
        let foodData: EnrichedFood | null = await getEnrichedFood(fdcId);
        if (!foodData) {
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSave)} className="space-y-4 max-h-[80vh] overflow-y-auto p-2">
