...
export async function getEnrichedFood(fdcId: number): Promise<{ success: boolean, data?: EnrichedFood, error?: string }> {
    ...
    try {
        ...
        if (docSnap.exists) {
            const food = convertTimestampsToISO(docSnap.data()) as EnrichedFood;
            return { success: true, data: food };
        }

        const foodDetails = await getFoodDetails(fdcId);
        if (!foodDetails) {
            return { success: false, error: 'Failed to get food details from USDA.' };
        }
        ...
        const aiResult = await enrichFoodDetailsFlow(aiInput);
        ...
        const finalFood = convertTimestampsToISO(newEnrichedFood);
        return { success: true, data: finalFood };

    } catch (error) {
        console.error(`[getEnrichedFood] Critical error for fdcId ${fdcId}:`, error);
